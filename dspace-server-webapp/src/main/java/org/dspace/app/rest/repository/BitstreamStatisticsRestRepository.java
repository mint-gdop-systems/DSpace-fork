...
public class BitstreamStatisticsRestRepository extends DSpaceRestRepository<BitstreamStatisticsRest, String> {
    ...
    public BitstreamStatisticsRest getStatistics(
        ...
        java.time.LocalDate approvedDate = null;
        if (org.apache.commons.lang3.StringUtils.isNotBlank(date)) {
            try {
                approvedDate = java.time.LocalDate.parse(date);
        ...
        try {
            ...
            List<Object[]> bitstreamResults = bitstreamService.findCount(context, submitterPerson, approvedDate);
            ...
            for (Object[] result : bitstreamResults) {
                ...
                } else if ("DOCUMENT_TYPE".equals(breakdownType)) {
                    BitstreamStatisticsRest.Breakdown breakdown = new BitstreamStatisticsRest.Breakdown();
                    breakdown.setBitstreams(bitstreamCount);
                    breakdown.setPdfPages(pageCount);
                    statisticsRest.getDocumentTypeBreakdown().put(category, breakdown);
                }
            }
